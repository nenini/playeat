...
export interface BossBattleSummary { battleId: number; guildId: number; bossId: number; bossName: string; difficulty: BossDifficulty; bossImageUrl?: string; status: BossBattleStatus; maxHp: number; currentHp: number; totalDamage: number; startedAt?: string; endedAt?: string; endsAt?: string; rewardClaimed: boolean }
