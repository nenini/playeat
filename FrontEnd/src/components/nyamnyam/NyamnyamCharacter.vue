<template>
  <div
    class="nyam-character"
    :style="sizeStyle"
    :class="[`appearance-${normalizedAppearance}`, `stage-${normalizedStage}`]"
  >
    <img
      class="nyam-image"
      :src="imageSrc"
...
const imageModules = {
  NYAMNYAM: import.meta.glob("../../assets/nyamnyam/*.png", {
    eager: true,
    query: "?url",
    import: "default",
  }) as Record<string, string>,
  PENGUIN: import.meta.glob("../../assets/penguin/*.png", {
    eager: true,
    query: "?url",
    import: "default",
  }) as Record<string, string>,
  DOG: import.meta.glob("../../assets/dog/*.png", {
    eager: true,
    query: "?url",
    import: "default",
  }) as Record<string, string>,
...
const normalizedStage = computed<Stage>(() =>
  normalizeOption(props.stage, validStages, "baby"),
);
const normalizedMood = computed<NyamnyamMood>(() =>
  normalizeOption(props.mood, validMoods, "normal"),
);
const normalizedAppearance = computed<AppearanceType>(() =>
  normalizeAppearance(props.appearanceType),
);
const imageKey = computed(
  () => `${normalizedStage.value}-${normalizedMood.value}`,
);
const fallbackKey = "baby-normal";
const imageSrc = computed(() => {
  const key = imageLoadFailed.value ? fallbackKey : imageKey.value;
  return (
    imageForKey(key, normalizedAppearance.value) ||
    imageForKey("egg-normal", normalizedAppearance.value) ||
    imageForKey(fallbackKey, normalizedAppearance.value) ||
    imageForKey(fallbackKey, "NYAMNYAM") ||
    ""
  );
});
...
const resolvedHatImageUrl = computed(
  () => resolveItemAssetByKey(props.hatImageUrl) || props.hatImageUrl || null,
);
...
function imageForKey(key: string, appearance: AppearanceType) {
  ...
  const entry = Object.entries(modules).find(
    ([path]) =>
      path.endsWith(`${key}.png`) || path.endsWith(`${underscoreKey}.png`),
  );
  return entry?.[1];
}
</script>
